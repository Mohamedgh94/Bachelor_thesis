...
def uniMib_main():
    ...
    model = GatedTransformer(input_dim=config["input_size"],d_model=256, num_heads=8, d_ff=512, num_layers=4,config = config,num_classes=config['num_classes'] ,dropout_rate=0.3)
...
def sisFall_main():
    config = configuration(dataset_idx=1, dataset_paths = 'SisFall',output_idx=0, 
                        gpudevice_idx=1,usage_mod_idx= 1 , learning_rates_idx=1,batch_size_idx=2 ,input_size_idx= 1,
                            epochs=5)
