...
from SaveAndLoadModel import SaveAndLoadModel
from MultiTaskLossFunction import MultiTaskLossFunction as loss
...
if __name__ == "__main__":
    ...
    model = GatedTransformer(input_dim=6, d_model=512, num_heads=16, d_ff=1024, num_layers=6)
    loss_fn = loss()
...
""" class Main:
    def __init__(self, model, loss_fn):
        ...
        self.model_saver = SaveAndLoadModel(self.model,loss_fn=self.loss_fn,learning_rate=self.learning_rate,optimizer_class= torch.optim.Adam,epochs=self.epochs ,model_path= self.model_path, device=self.device)
    ...
    def run(self):
        ...
        if choice == '0':
            self.model_saver.train(self.train_loader, self.valid_loader)
        if choice == '1':
